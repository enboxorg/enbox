...
export function testNestedRoleScenarios(): void {
  describe('Nested role scenarios', () => {
    ...
    it('should support Slack-like protocol with community and gated channels', async () => {
      ...
      expect(bobCommunityReadReply.entry!.recordsWrite?.recordId).to.equal(communityRecord.message.recordId);
      ...
      expect(carolReadReply.entry!.recordsWrite?.recordId).to.equal(channel1Record.message.recordId);
