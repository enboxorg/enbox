...
export class EncryptionProtocol implements CoreProtocol {
  ...
  public async validateRecord(message: RecordsWriteMessage, dataBytes: Uint8Array): Promise<void> {
    if (message.descriptor.protocolPath === EncryptionProtocol.audienceEpochPath) {
      const dataObject = Encoder.bytesToObject(dataBytes) as AudienceEpochData;
      validateJsonSchema('AudienceEpoch', dataObject);
      await EncryptionProtocol.validateAudienceEpoch(message, dataObject);
      return;
    }

    if (message.descriptor.protocolPath === EncryptionProtocol.audienceKeyPath ||
      message.descriptor.protocolPath === EncryptionProtocol.grantKeyPath) {
  ...
  private static async validateAudienceEpoch(message: RecordsWriteMessage, data: AudienceEpochData): Promise<void> {
    EncryptionProtocol.validateTags(message.descriptor.tags, {
    ...
    await EncryptionProtocol.validateKeyId(data.keyId, data.publicKeyJwk);
  }

  private static validateTags(actualTags: RecordsWriteTags | undefined, expectedTags: RecordsWriteTags): void {
    for (const [key, expectedValue] of Object.entries(expectedTags)) {
      if (actualTags?.[key] !== expectedValue) {
        throw new DwnError(
          DwnErrorCode.EncryptionProtocolValidateTagsMismatch,
          `Encryption Protocol tag '${key}' does not match payload.`
        );
      }
    }
  }

  private static async validateKeyId(keyId: string, publicKeyJwk: PublicKeyJwk): Promise<void> {
    const computedKeyId = await Encryption.getKeyId(publicKeyJwk);
    if (computedKeyId !== keyId) {
      throw new DwnError(
        DwnErrorCode.EncryptionProtocolValidateKeyIdMismatch,
        `keyId ${keyId} does not match publicKeyJwk thumbprint ${computedKeyId}.`
      );
    }
  }
}
