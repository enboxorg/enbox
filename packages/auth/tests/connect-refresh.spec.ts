...
describe('delegated connection lifecycle', () => {
  ...
  describe('AuthManager connection monitor', () => {
    ...
    test.each([
      ...
      const clock = sinon.useFakeTimers();
      const agent = createMockAgent();
      const removeInactive = sinon.spy(agent.sync, 'removeIdentityIfApprovalInactive');
      const manager = createTestManager(agent);
      const previousSession = manager.session;
      ...
      try {
        ...
        expect(removeInactive.callCount).toBe(state === 'expired' ? 1 : 0);
        if (state === 'expired') {
          expect(removeInactive.calledOnceWithExactly({ did: OWNER_DID, delegateDid: DELEGATE_DID, connectSessionId: 'session-1' })).toBe(true);
        }
    ...
    test('does not publish expiry or auto-refresh when sync rejects the approval as stale', async () => {
      const clock = sinon.useFakeTimers();
      const agent = createMockAgent();
      const removeInactive = sinon.stub(agent.sync, 'removeIdentityIfApprovalInactive').resolves(false);
      const manager = createTestManager(agent);
      ...
      try {
        await clock.tickAsync(0);
        expect(removeInactive.calledOnce).toBe(true);
        expect(manager.session).toBe(session);
    ...
    test('does not overlap slow polls or apply a result from a replaced session', async () => {
      ...
      expect(removeInactive.notCalled).toBe(true);
      expect(expired.notCalled).toBe(true);
      stop();
      clock.restore();
    });
