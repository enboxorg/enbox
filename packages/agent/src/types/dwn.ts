...
export type ProcessDwnRequest<T extends DwnInterface> = DwnRequest<T> & {
   ...
   * `RecordsQuery` replies are decrypted in place; for `RecordsSubscribe` both
   * the reply's initial snapshot entries and each event's inline `encodedData`
   * payload are decrypted before delivery; and for `MessagesSubscribe` each
   * event's inline `RecordsWrite` `encodedData` is decrypted the same way
   * (other message types on the feed carry nothing to decrypt). Events whose
   * data was too large to be inlined carry no payload to decrypt — the
   * per-record lazy read (with decryption) remains the data path for those. A
   * snapshot entry or event that cannot be decrypted never fails the subscribe
   * or kills the subscription: its inline ciphertext is withheld instead,
   * leaving the lazy read to surface the decryption error on data access.
   */
  encryption?: boolean;
  /**
