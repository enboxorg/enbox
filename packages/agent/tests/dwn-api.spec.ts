...
import { BearerDid, DidDht, DwnEndpointResolutionErrorCode } from '@enbox/dids';
import {
...
describe('AgentDwnApi', () => {
  ...
  describe('getDwnEndpointUrlsForTarget()', () => {
    ...
    function createResolveResult(did: string, serviceEndpoint: string | string[]): object {
      return {
        didDocument: {
          id      : did,
          service : [{
    ...
    function createMockAgent(): any {
      return {
        ...
        did      : {
          resolve: sinon.stub().resolves(createResolveResult(localDid, 'https://remote.example')),
        },
        identity: {
    ...
    it('should use the local DWN endpoint if available even when DID resolution fails', async () => {
      const mockAgent = createMockAgent();
      mockAgent.did.resolve.rejects(new Error('DID resolution failed'));
      ...
      expect(mockAgent.did.resolve.callCount).toBe(1);
    });
    ...
    it('should throw when strategy is only and local DWN is unavailable', async () => {
      ...
      expect(mockAgent.did.resolve.callCount).toBe(0);
    });
    ...
    it('should prepend local endpoint ahead of DID endpoints when strategy is prefer', async () => {
      const mockAgent = createMockAgent();
      mockAgent.did.resolve.resolves(
        createResolveResult(localDid, ['https://remote-a.example', 'https://remote-b.example'])
      );
      mockAgent.rpc.getServerInfo.resolves({ server: '@enbox/dwn-server' });
    ...
    it('should adopt a newly managed connected DID after topology invalidation', async () => {
      const mockAgent = createMockAgent();
      mockAgent.identity.list.resolves([]);
      mockAgent.rpc.getServerInfo.resolves({ server: '@enbox/dwn-server' });
      const dwnApi = new AgentDwnApi({
      ...
      await expect(dwnApi.getDwnEndpointUrlsForTarget(localDid))
        .resolves.toEqual(['https://remote.example']);

      mockAgent.identity.list.resolves([{
        did      : { uri: 'did:jwk:delegate' },
        metadata : { connectedDid: localDid },
      }]);
      dwnApi.invalidateLocalManagedDidCache();

      await expect(dwnApi.getDwnEndpointUrlsForTarget(localDid)).resolves.toEqual([
        'http://127.0.0.1:3000',
        'https://remote.example',
      ]);
    });

    it('should stop treating a deleted identity as locally managed after topology invalidation', async () => {
      const mockAgent = createMockAgent();
      mockAgent.rpc.getServerInfo.resolves({ server: '@enbox/dwn-server' });
      const dwnApi = new AgentDwnApi({
        agent            : mockAgent,
  ...
  describe('sendRequest()', () => {
    ...
    beforeAll(async () => {
      ...
      // Portable identity import treats resolution as authoritative. Publish the fixture first so
      // the relay document and supplied private keys describe the same verification methods.
    ...
    it('should use a secure (wss) transport when the dwnUrl is also secure (https)', async () => {
      ...
      // mock DID resolution to return a DWN service endpoint that is secure (https)
      sinon.stub(testHarness.agent.did, 'resolve').resolves({
        didDocument: {
          id      : alice.did.uri,
          service : [{
            id              : `${alice.did.uri}#dwn`,
            type            : 'DecentralizedWebNode',
    ...
    it('should use a non-secure (ws) transport when the dwnUrl is also non-secure (http)', async () => {
      ...
      // mock DID resolution to return a DWN service endpoint that is insecure (http)
      sinon.stub(testHarness.agent.did, 'resolve').resolves({
        didDocument: {
          id      : alice.did.uri,
          service : [{
    ...
    it('throws an error if target DID has no #dwn service endpoints', async () => {
      ...
      try {
        ...
        expect(error).toMatchObject({ code: DwnEndpointResolutionErrorCode.ServiceMissing });
      }
    });
...
describe('Encryption Callback Factories', () => {
  ...
  describe('fetchRemoteProtocolDefinition()', () => {
    ...
    it('should delegate to the standalone function and return the definition', async () => {
      ...
      const mixedEndpoints = sinon.stub(dwnApi, 'getDwnEndpointUrlsForTarget').resolves(['http://localhost:3000']);
      const remoteEndpoints = sinon.stub(dwnApi, 'getRemoteDwnEndpointUrls').resolves(['https://hosted.example']);
      const sendDwnRpcRequest = sinon.stub(dwnApi as any, 'sendDwnRpcRequest')
        .callsFake(async (params: any): Promise<any> => {
      ...
      expect(remoteEndpoints.calledOnceWithExactly(alice.did.uri)).toBe(true);
...
describe('Role record write behavior', () => {
  ...
  it('rejects a supplied Ed25519 (or malformed) role key instead of falsely reporting delivered', async () => {
    ...
    const nonCanonicalKey = { kty: 'OKP', crv: 'X25519', x: `${VALID_X25519_KEY.x}=` } as any;
    await expect(testHarness.agent.dwn.processRequest(roleParamsWithKey(nonCanonicalKey)))
  ...
  it('rejects recipientRolePublicKey on unsupported paths (sendRequest / raw / non-RecordsWrite / store:false)', async () => {
    await testHarness.agent.dwn.processRequest({
      author        : alice.did.uri,
      target        : alice.did.uri,
      messageType   : DwnInterface.ProtocolsConfigure,
    ...
    await expect(testHarness.agent.dwn.processRequest({
      ...
      recipientRolePublicKey : VALID_X25519_KEY,
    })).rejects.toThrow(/only supported when writing .*\$role record|non-raw, stored RecordsWrite/i);
    ...
    // `store: false` persists nothing to deliver against, so a supplied key there
    // would provision a delivery for a role record that was never stored — reject it.
    const { message: threadForStoreFalse } = await testHarness.agent.dwn.processRequest({
      author        : alice.did.uri,
      target        : alice.did.uri,
      messageType   : DwnInterface.RecordsWrite,
      messageParams : { protocol: chatProtocol.protocol, protocolPath: 'thread', dataFormat: 'application/json', schema: 'https://schemas.xyz/thread' },
    ...
    await expect(testHarness.agent.dwn.processRequest({
      ...
      messageParams : {
        ...
        protocol        : chatProtocol.protocol,
        protocolPath    : 'thread/participant',
        parentContextId : (threadForStoreFalse as RecordsWriteMessage).contextId,
        dataFormat      : 'application/json',
        schema          : 'https://schemas.xyz/participant',
      },
      dataStream             : new Blob([new TextEncoder().encode('{"name":"Alice"}')]),
      store                  : false,
      recipientRolePublicKey : VALID_X25519_KEY,
  ...
  it('keeps accepted role writes non-blocking and wakes matching protocol views after projection persistence', async () => {
    ...
    const bob = await testHarness.createIdentity({ name: 'Bob Supplied', testDwnUrls });
    ...
    const deliveries = await testHarness.agent.dwn.processRequest({
      ...
      target        : alice.did.uri,
      messageType   : DwnInterface.RecordsQuery,
      messageParams : { filter: { protocol: chatProtocol.protocol, protocolPath: ENCRYPTION_CONTROL_DELIVERY_PATH, recipient: bob.did.uri } },
    });
    expect(deliveries.reply.entries ?? []).toHaveLength(1);
  }, 15000);
  ...
  it('rejects a supplied key up front when the path is not a deliverable $role recipient', async () => {
    const bob = await testHarness.createIdentity({ name: 'Bob Preflight', testDwnUrls });

    // Protocol not installed on the target: nothing to resolve $role/$keyAgreement from.
    await expect(testHarness.agent.dwn.processRequest({
      author                 : alice.did.uri,
      target                 : alice.did.uri,
      messageType            : DwnInterface.RecordsWrite,
      messageParams          : { recipient: bob.did.uri, protocol: chatProtocol.protocol, protocolPath: 'thread/participant', dataFormat: 'application/json', schema: 'https://schemas.xyz/participant' },
      dataStream             : new Blob([new TextEncoder().encode('{"name":"Bob"}')]),
      recipientRolePublicKey : VALID_X25519_KEY,
    })).rejects.toThrow(/is not installed on target/i);
    ...
    await expect(testHarness.agent.dwn.processRequest({
      ...
      messageType            : DwnInterface.RecordsWrite,
      messageParams          : { protocol: chatProtocol.protocol, protocolPath: 'thread', dataFormat: 'application/json', schema: 'https://schemas.xyz/thread' },
      dataStream             : new Blob([new TextEncoder().encode('{"title":"NoRecipient"}')]),
      recipientRolePublicKey : VALID_X25519_KEY,
    })).rejects.toThrow(/protocol, protocolPath, and recipient/i);

    // A non-$role path (thread): delivery only applies to $role records.
    const { message: threadMessage } = await testHarness.agent.dwn.processRequest({
      author        : alice.did.uri,
      target        : alice.did.uri,
      messageType   : DwnInterface.RecordsWrite,
    ...
    await expect(testHarness.agent.dwn.processRequest({
      ...
      dataStream             : new Blob([new TextEncoder().encode('hi')]),
      recipientRolePublicKey : VALID_X25519_KEY,
    })).rejects.toThrow(/is not a\s*\$role path/i);
  }, 15000);
