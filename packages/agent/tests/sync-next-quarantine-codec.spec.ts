...
import { syncNextReceiptKey } from '../src/sync-next/ledger-key.js';
import {
...
describe('SyncNext quarantine codec', () => {
  ...
  it('should reject an entry from another position even when its CID matches', async () => {
    const binding = { identity: identity(), messageCid: 'cid-1', source: source('2') };
    await expect(sealSyncNextQuarantinePayload(vault, binding, entry()))
      .rejects.toThrow('position does not match its receipt');

    const encrypted = await vault.encryptData({
    ...
    await expect(openSyncNextQuarantinePayload(vault, binding, encrypted))
      .rejects.toThrow('position does not match its receipt');
  });
