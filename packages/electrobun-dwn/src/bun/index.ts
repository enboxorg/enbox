...
function createDwnServerConfig(port: number): DwnServerConfig {
	return {
		...
		maxRecordDataSize: 1_073_741_824, // 1gb
		webSocketSupport,
		eventStreamPluginPath: process.env.DWN_EVENT_STREAM_PLUGIN_PATH,
		messageStore:
			process.env.DWN_STORAGE_MESSAGES || process.env.DWN_STORAGE || "level://data",
		dataStore:
			process.env.DWN_STORAGE_DATA || process.env.DWN_STORAGE || "level://data",
		stateIndex:
			process.env.DWN_STORAGE_STATE_INDEX || process.env.DWN_STORAGE || "level://data",
		resumableTaskStore:
