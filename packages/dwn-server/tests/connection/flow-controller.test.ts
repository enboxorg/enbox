...
/** Helper to create a SubscriptionEose message with a given position. */
function makeEose(pos: number): SubscriptionMessage {
  return { type: 'eose', cursor: token(pos) };
}
...
describe('FlowController', () => {
  ...
  describe('push()', () => {
    it('should send events immediately when window has room', () => {
      ...
      expect(sent[0].result.subscription.cursor).toEqual(token(1));
      expect(sent[1].result.subscription.cursor).toEqual(token(2));
  ...
  describe('ack()', () => {
    ...
    it('should handle cumulative acks (ack last cursor in batch)', () => {
      ...
      // Cumulative ack up to token(3) — frees all 3 slots, flushes 3 buffered
      fc.ack(token(3));
