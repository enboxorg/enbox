...
| The connectivity-driven recovery check: skips links whose replication stream is current (`isStreamCurrent`, both directions), reconciles stale links on their mailbox lanes under identity task supervision — endpoint-grouped like a full run — and falls back to a full verifying `sync()` only when a target has no active controller. Skips entirely while a sync run is in flight. No orphan re-init. | **convergence check** — `runConvergenceCheck`, `scheduleConvergenceCheck` | `runIntegrityCheck`, `scheduleIntegrityCheck` |
