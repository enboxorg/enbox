...
The agent's pull path is `admitClosure()` (`packages/agent/src/sync-admit-closure.ts`); the push path is its mirror in `sync-next/push-page.ts` using `sync-messages.ts`. Both follow the same loop:
