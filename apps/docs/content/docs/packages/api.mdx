...
const { records, cursor } = await tasks.records.query('task', selection);

// Fetch next page
if (cursor) {
  const { records: next } = await tasks.records.query('task', {
    ...selection,
    pagination: { ...selection.pagination, cursor },
  });
}
```
...
returned record and the original instance reflect the updated state.
...
```ts
const { status, record: updated } = await record.patch({
  done: true,
});
```
