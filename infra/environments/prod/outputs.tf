...
output "secret_sync_lambda_name" {
  description = "Name of the Lambda that keeps dwn/prod/database-url in sync with the Aurora master secret."
...
output "post_apply_instructions" {
  ...
  value       = <<-EOT
    1. Populate dwn/prod/database-url from the Aurora master secret (idempotent;
